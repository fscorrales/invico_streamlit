...
Date   : 05-ago-2026
Purpose: Migrate SIIF's rfp_p605b () from SQLite to MongoDB
"""

__all__ = ["sync_rfp_p605b_to_mongodb"]
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate SIIF's rfp_p605b () from SQLite to MongoDB",
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to the SQLite database file containing the rf610 data to migrate (also can be set with the environment variable SQLITE_PATH)",
    )
...
def migrate_df_to_mongodb(endpoint: str, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        print(f"Successfully migrated Rfp_p605b's {len(records)} records to MongoDB.")
...
# --------------------------------------------------
def sync_rfp_p605b_to_mongodb(sqlite_path: str, endpoint: str) -> None:
    """Download, process and sync the rfp_p605b report to the repository."""
    try:
...
def main():
    ...
    sync_rfp_p605b_to_mongodb(
        sqlite_path=args.sqlite_path,
        endpoint=Endpoints.SIIF_RFP_P605B.value,
    )
...
if __name__ == "__main__":
    ...
    # poetry run python -m src.migration.siif.rfp_p605b -s "D:\Datos INVICO\IT\Python\Python Output\SQLite Files\siif.sqlite"
