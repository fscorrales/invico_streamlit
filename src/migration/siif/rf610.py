...
Date   : 14-jul-2026
Purpose: Migrate SIIF's rf610 (Prespuesto de Gastos con Descripción) from SQLite to MongoDB
"""

__all__ = ["sync_rf610_to_mongodb"]
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate SIIF's rf610 (Prespuesto de Gastos con Descripción) from SQLite to MongoDB",
...
def migrate_df_to_mongodb(endpoint: str, df: pd.DataFrame) -> None:
    ...
    try:
        ...
        print(f"Successfully migrated Rf610's {len(records)} records to MongoDB.")
...
# --------------------------------------------------
def sync_rf610_to_mongodb(sqlite_path: str, endpoint: str) -> None:
    """Download, process and sync the rf602 report to the repository."""
    try:
...
if __name__ == "__main__":
    ...
    # poetry run python -m src.migration.siif.rf610
