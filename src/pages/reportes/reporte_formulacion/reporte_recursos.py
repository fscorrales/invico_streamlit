...
from src.services import get_ejercicios, get_reporte_formulacion_recursos
...
ENDPONT = Endpoints.REPORTE_FORMULACION_RECURSOS.value
REPORTE = "reporte_formulacion_recursos"
...
def render(automation_func: Optional[Any] = None) -> None:
    ...
    try:
        df = get_reporte_formulacion_recursos(
            filtro_actual,
    ...
    if not df.empty:
        ...
        dataframe(
            df,
            key=f"{REPORTE}_df_reporte_formulacion_recursos",
